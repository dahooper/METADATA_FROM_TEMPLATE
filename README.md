...
* The value of _variables_ must be a nested list of variable names. Each of these begins on a new line, has no explicit indentation, is preceded by a hyphen - and one or more white spaces, and is followed (with no intervening white spaces) by a colon : but no other characters.
  ...
  * Each attribute name acts as the key in a key: value pair, whose value will be a nested dictionary containing keys data_type and value. Consequently, variable attributes are teated in an identical way to global attributes - see above - except that they have an additional level of indentation.
